 ...
 * return true/false
 */
class Fvidel3Controller extends Controller {
	...
	public function yiji_check2(){
		...
		if(!$param){
            ...
            $cid = I('get.cid');
			$count=M("slskillclass")->where("skc_fid=3 AND skc_id<>$cid AND skc_title='$param'")->count();
			if($count){//存在，验证失败
				$status="n";
				$info="此名称已存在";
			}else{//不存在，验证成功
	...
	public function yiji_check(){
		...
		if(!$param){
			...
			$count=M("slskillclass")->where("skc_fid=3 AND skc_title='$param'")->count();
			if($count){//存在，验证失败
				$status="n";
				$info="此名称已存在";
			}else{//不存在，验证成功
